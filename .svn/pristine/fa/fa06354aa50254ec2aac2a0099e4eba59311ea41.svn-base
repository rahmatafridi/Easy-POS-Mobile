                ...
                function (value) {
                    if (value != 'error') {
                        ...
                        if (value == 'SESSION EXPIRED') {
                            ...
                            $('#tblCustomer tbody tr').each(function (i, obj) {
...
var LoadGridPayments = function () {
    $.ajax({
        type: "POST",
        cache: false,
        url: "../CustomerManagement/LoadGridPayments",
        processData: false,
        contentType: false,
        success: function (data) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                html += ' </button></td>';
                html += '<td>';
                html += '<button id=' + obj.UID + ' class="btn btn-danger btn-block btn-xs btn-payToCustomer">';
                html += '<i class="fa fa-edit"></i>';
                html += ' Pay';
                html += ' </button></td>';
                html += '</tr>'
...
$(document).on('click', '.btn-payToCustomer', function () {
    ...
    $.ajax({
        type: "POST",
        cache: false,
        url: "../CustomerManagement/LoadCustomerByUID",
        data: data,
        processData: false,
        contentType: false,
        success: function (Rdata) {
            if (Rdata != 'error') {
                ...
                $('#pContact').removeAttr('disabled');
                $('#pBalance').removeAttr('disabled');

                $('#pName').val(obj.Name);
                $('#pContact').val(obj.Contact);
                $('#pBalance').val(obj.Balance);

                $('#pName').attr('disabled', true);
                $('#pContact').attr('disabled', true);
                $('#pBalance').attr('disabled', true);
...
$('#btnSave').on('click', function () {
    ...
    $.ajax({
        type: "POST",
        cache: false,
        url: "../CustomerManagement/PayToCustomer",
        data: data,
        processData: false,
        contentType: false,
        success: function (Rdata) {
            if (Rdata != 'error') {
                ...
                $('#tblCustomer').DataTable().draw();
                    ...
                    swal("success", " Payment Recieved Successfully", "success");
                    $('#myModal').modal('hide');

                    var url = '../ASPXPages/CustomerReceivedReceipt.aspx?UID=' + data.UID + '&IsPrint=1&CpId=' + data.CpId;
                    openNewWin(url);
...
$('#pbtnSave').on('click', function () {

    if ($('#PayToCustomer').val() == "" || $('#PayToCustomer').val() < 1) {
        swal("warning", " Please Enter Rupees", "warning");
        return;
    }

    var p = parseInt($('#PayToCustomer').val());
    var b = parseInt($('#pBalance').val());
    if ( p > b ) {
        swal("warning", " You Can't pay more than balance", "warning");
        return;
    }
    ...
    $.ajax({
        type: "POST",
        cache: false,
        url: "../CustomerManagement/PaymentToCustomer",
        data: data,
        processData: false,
        contentType: false,
        success: function (Rdata) {
